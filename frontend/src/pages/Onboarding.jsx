...
const Onboarding = () => {
        ...
        onAuthStateChanged(authInstance, async (user) => {
          if (user) {
            ...
            if (userDoc.exists() && userDoc.data().registrationComplete) {
              navigate('/'); // Redirect to home if onboarding is complete
            }
            setLoading(false);
          } else {
  ...
  // useEffect(() => {
  //   // Fetch courses from Blackboard on initial load
  //   if (currentUser && !loading) {
  //     fetchCoursesFromBlackboard();
  //   }
  // }, [currentUser, loading]);
  ...
  // const fetchCoursesFromBlackboard = async () => {
  //   setLoading(true);
  //   try {
  //     const response = await fetch('http://127.0.0.1:3030/scrape');
  //     const scraped_courses = await response.json();
  //     
  //     const course_info = scraped_courses.courses.map((course) => ({
  ...
  //     setError('Failed to fetch courses from Blackboard. Please add them manually.');
  //   } finally {
  //     setLoading(false);
  //   }
  // };
  ...
  const handleSubmit = async (e) => {
    ...
    const validCourses = userCourses.every(isValidCourse);
    if (!validCourses) {
      setError('Please complete all course information');
      return;
    }
    ...
    try {
      ...
      await updateDoc(doc(firestoreRef.current, 'users', currentUser.uid), {
        firstName,
        lastName,
        username,
        courses: userCourses,
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            <h3 className="text-lg font-medium text-nexus-blue-700">Your Courses</h3>
            {/* <button
              type="button"
              onClick={handleRefreshCourses}
              className="text-sm text-nexus-blue-600 hover:text-nexus-blue-800 underline"
            >
              Refresh from Blackboard
            </button> */}
          </motion.div>
          ...
          <motion.button
            type="submit"
            className="w-full mt-6 bg-nexus-blue-600 text-white rounded-md py-2 px-4 hover:bg-nexus-blue-700 focus:outline-none focus:ring-2 focus:ring-nexus-blue-500 focus:ring-opacity-50 text-base"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
