...
const Navbar = () => {
  ...
  const textColor = isScrolled ? 'text-nexus-blue-900' : 'text-white';
  const navbarClasses = `fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
    isScrolled ? 'bg-white bg-opacity-90 shadow-md navbar-blur' : 'bg-gradient-to-r from-nexus800 to-nexus900'
  }`;

  const linkClasses = `flex items-center justify-center md:justify-start gap-1 md:gap-2 text-white md:text-sm text-2xl md:text-base cursor-pointer`;
      ...
      <div className="container mx-auto">
        <div className="flex justify-between items-center py-5 md:py-4 px-3 md:px-0">
          ...
          >
            ...
            />
            <span className={`text-xl font-bold hidden md:inline ml-2 ${isScrolled ? 'text-nexus-blue-900' : 'text-white'}`}>
              Nexus
            </span>
          </button>
          ...
          >
            <AiFillDiscord className={`text-4xl md:text-xl ml-5 md:ml-4 ${textColor}`} />
            <span className={`hidden md:inline ml-2 ${textColor}`}>Discord Servers</span>
          </Link>
          ...
          <Link
            ...
            className="flex justify-center items-center w-full md:w-auto"
          >
            <FaCalculator className={`text-3xl md:text-lg ml-5 md:ml-4 ${textColor}`} />
            <span className={`hidden md:inline ml-2 ${textColor}`}>Grade Calculator</span>
          </Link>

          <button
            onClick={handleSuperdocClick}
            className="flex justify-center items-center w-full md:w-auto">
            <HiDocumentText className={`text-4xl md:text-xl ml-5 md:ml-4 ${textColor}`} />
            <span className={`hidden md:inline ml-2 ${textColor}`}>Superdoc</span>
          </button>
          ...
          <div className="flex justify-center w-full md:w-auto ml-0 md:ml-0">
            {loading ? (
              ...
              <AvatarMenu buttonTone={isScrolled ? 'text-nexus-blue-900' : 'text-white'} />
            ) : (
              <Link
              to="/login"
              className={`flex font-bold py-1.5 px-3 text-sm md:text-base rounded border transition duration-300 transform hover:scale-110 ${
                isScrolled ? 'bg-blue-900 border-nexus-blue-900 text-white hover:bg-nexus-blue-100' 
                            : 'bg-white text-nexus-blue-900 hover:bg-gray-100'
              }`}
            >
